...
-- Table: season_metric_goals (specific targets for metrics)
CREATE TABLE season_metric_goals (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  season_area_metric_id uuid REFERENCES season_area_metrics(id) ON DELETE CASCADE NOT NULL,
  goal_value decimal NOT NULL, -- Value in user's preferred unit
  goal_unit text NOT NULL CHECK (goal_unit IN ('kg', 'lbs', 'cm', 'inches', 'meters', 'km', 'miles', 'seconds', 'minutes', 'hours', 'count', 'percent_value', 'custom_unit')),
  canonical_value decimal NOT NULL, -- Value converted to metric's default unit
  start_value decimal, -- Optional starting baseline value
  start_unit text CHECK (start_unit IN ('kg', 'lbs', 'cm', 'inches', 'meters', 'km', 'miles', 'seconds', 'minutes', 'hours', 'count', 'percent_value', 'custom_unit')),
  target_date timestamptz, -- Optional deadline for this goal
...
-- Table: metric_progress_logs (time-series progress tracking)
CREATE TABLE metric_progress_logs (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  season_id uuid REFERENCES seasons(id) ON DELETE CASCADE NOT NULL,
  metric_id uuid REFERENCES metrics(id) ON DELETE CASCADE NOT NULL,
  user_id uuid REFERENCES auth.users(id) NOT NULL,
  value decimal NOT NULL,
  unit text NOT NULL CHECK (unit IN ('kg', 'lbs', 'cm', 'inches', 'meters', 'km', 'miles', 'seconds', 'minutes', 'hours', 'count', 'percent_value', 'custom_unit')),
  canonical_value decimal NOT NULL, -- Value converted to metric's default unit
  logged_at timestamptz NOT NULL DEFAULT now(),
  entry_type text NOT NULL CHECK (entry_type IN ('manual', 'app_calculated', 'imported')) DEFAULT 'manual',
  source_reference text, -- Reference to source data (e.g., workout_session_id for calculated metrics)
  notes text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
CREATE INDEX idx_season_area_metrics_season_pillar_area_id ON season_area_metrics(season_pillar_area_id);
CREATE INDEX idx_season_metric_goals_season_area_metric_id ON season_metric_goals(season_area_metric_id);

-- Progress tracking indexes
CREATE INDEX idx_metric_progress_logs_season_metric ON metric_progress_logs(season_id, metric_id);
CREATE INDEX idx_metric_progress_logs_user_id ON metric_progress_logs(user_id);
CREATE INDEX idx_metric_progress_logs_logged_at ON metric_progress_logs(logged_at);
CREATE INDEX idx_metric_progress_logs_user_logged_at ON metric_progress_logs(user_id, logged_at DESC);
...
ALTER TABLE season_metric_goals ENABLE row level security;
ALTER TABLE metric_progress_logs ENABLE row level security;
...
-- RLS Policies for season_area_metrics (access through season ownership)
CREATE POLICY "Users can access metrics for their own seasons" ON season_area_metrics FOR
    SELECT USING ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
        WHERE spa.id = season_area_metrics.season_pillar_area_id
    ));

CREATE POLICY "Users can create metrics for their own seasons" ON season_area_metrics FOR
    INSERT WITH CHECK ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
...
CREATE POLICY "Users can update metrics for their own seasons" ON season_area_metrics FOR
    UPDATE USING ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
...
CREATE POLICY "Users can delete metrics for their own seasons" ON season_area_metrics FOR
    DELETE USING ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
...
-- RLS Policies for season_metric_goals (access through season ownership)
CREATE POLICY "Users can access goals for their own seasons" ON season_metric_goals FOR
    SELECT USING ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
        JOIN season_area_metrics sam ON spa.id = sam.season_pillar_area_id
        WHERE sam.id = season_metric_goals.season_area_metric_id
    ));

CREATE POLICY "Users can create goals for their own seasons" ON season_metric_goals FOR
    INSERT WITH CHECK ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
...
CREATE POLICY "Users can update goals for their own seasons" ON season_metric_goals FOR
    UPDATE USING ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
...
CREATE POLICY "Users can delete goals for their own seasons" ON season_metric_goals FOR
    DELETE USING ((SELECT auth.uid()) = (
        SELECT s.user_id FROM seasons s 
        JOIN season_pillars sp ON s.id = sp.season_id 
        JOIN season_pillar_areas spa ON sp.id = spa.season_pillar_id
...
-- RLS Policies for metric_progress_logs
CREATE POLICY "Users can view their own progress logs" ON metric_progress_logs FOR
    SELECT USING ((SELECT auth.uid()) = user_id);

CREATE POLICY "Users can create their own progress logs" ON metric_progress_logs FOR
    INSERT WITH CHECK ((SELECT auth.uid()) = user_id);

CREATE POLICY "Users can update their own progress logs" ON metric_progress_logs FOR
    UPDATE USING ((SELECT auth.uid()) = user_id);

CREATE POLICY "Users can delete their own progress logs" ON metric_progress_logs FOR
    DELETE USING ((SELECT auth.uid()) = user_id);
...
CREATE TRIGGER handle_times_season_metric_goals
    BEFORE INSERT OR UPDATE ON season_metric_goals
    FOR EACH ROW
EXECUTE PROCEDURE handle_times();

CREATE TRIGGER handle_times_metric_progress_logs
    BEFORE INSERT OR UPDATE ON metric_progress_logs
    FOR EACH ROW
EXECUTE PROCEDURE handle_times();
...
COMMENT ON TABLE season_area_metrics IS 'Which metrics track each area of focus';
COMMENT ON TABLE season_metric_goals IS 'Specific targets and goals for metrics';
COMMENT ON TABLE metric_progress_logs IS 'Time-series progress tracking for all metrics';
...
COMMENT ON COLUMN season_metric_goals.canonical_value IS 'Goal value converted to metric default unit for comparison';
COMMENT ON COLUMN metric_progress_logs.entry_type IS 'How the entry was created: manual, app_calculated, or imported';
COMMENT ON COLUMN metric_progress_logs.source_reference IS 'Reference to source data for calculated/imported entries';
...
ALTER PUBLICATION supabase_realtime ADD TABLE season_area_metrics;
ALTER PUBLICATION supabase_realtime ADD TABLE season_metric_goals;
ALTER PUBLICATION supabase_realtime ADD TABLE metric_progress_logs;
